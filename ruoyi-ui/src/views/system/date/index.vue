<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dateList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="活动时间" align="center" prop="id" v-if="false"/>
...
<script>
import { listDate, getDate, delDate, addDate, updateDate, exportDate } from "@/api/system/date";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除活动时间（分场次）编号为"' + ids + '"的数据项?', "警告", {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有活动时间（分场次）数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(() => {
